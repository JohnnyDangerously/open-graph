...
export default function HUD({ focus, nodes, fps, selectedIndex, concentric, onToggleConcentric, onSettings, onBack, onForward, canBack, canForward, filters, onToggleFilter }:{ focus: string | null, nodes: number, fps: number, selectedIndex?: number | null, concentric?: boolean, onToggleConcentric?: ()=>void, onSettings: ()=>void, onBack: ()=>void, onForward: ()=>void, canBack:boolean, canForward:boolean, filters:{email:boolean,work:boolean,social:boolean,phone:boolean}, onToggleFilter:(k:keyof typeof filters)=>void }){
  return (
    <div style={{ position:"absolute", top:10, right:10, zIndex:20, display:"flex", gap:8, alignItems:"center" }}>
      ...
      <div style={{ display:'flex', gap:6, marginLeft:6 }}>
        {([
          ...
          const active = (filters as any)[k]
          return <button key={k} onClick={()=>onToggleFilter(k)} style={{ padding:"8px 10px", borderRadius:8, background: active? '#4f7cff' : 'rgba(255,255,255,0.08)', color:'#fff', border:'1px solid rgba(255,255,255,0.15)' }}>{label}</button>
        })}
      </div>
      {onToggleConcentric && (
